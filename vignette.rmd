...
Next, in Collection Calendar's server function, a table is queried, then modified to contain desired fields, and desired rows based on the sidebar filters. Throughout the app, tables are queried with the suffix "_db". This version is kept, as it is the version that the app interacts with and reacts to changes. The "clean" displayable version lacks a suffix and reacts based on on "_db" version.  The datatable object `output$collection` receives `rv$collect_table` as an input., and is created with argument `selection = "single"`. This allows the user to select one row at a time; when the user does, the numeric index of the selected row is assigned to the reactive value `input$collection_rows_selected`.
