...
Clicking a row in the collection calendar brings the user to the Deploy Sensor page, with the SMP ID and row selected. There are several steps that happen within the Collection Calendar server function to get there, and some that happen on the Deploy Sensor page. In this first chunk, from the Collection Calendar server function, notably, the function argument `selection` = "single". `formatStyle` and `styleInterval` are used to highlight cells when the date within the cell has passed. 
...
Moving into the Deploy Sensor server function, the returned values from the collection calendar are in a list called `collect`. As noted, `collect$deploy_refresh` sets off certain updates in the `observeEvent`. Similar `observeEvent` functions exist in Deploy Sensor when coming from Special Investigation, SRT, Sensor, and Location modules. In Deploy Sensor, the SMP ID or Site Name is the foundational value. Tables and `selectInput` functions are based on the SMP ID or Site Name. So, they are first reset, and then a value is selected based on the `collect` list. Since `collect$deploy_refresh()` is initiated during the app load, the `observeEvent` will be triggered. That is why there is a length check; so it only follows through with this action if the SMP ID value actually exists. If it does, and then if the SMP ID is not blank, then it will be updated. If it is blank, the Site Name will be updated. Since those actions are not instant, a delay is placed before selecting a row in the accompanying datatable.  `collect$row` is the deployment_uid of the row selected in the collection calendar, and that is used to select the row in the deployment datatable. Selecting a row in the deployment datatable also pre-fills most sidebar fields for easy editing. More on that later. 
